...
if ($method == 'search_account_list') {
    ...
    $query = "SELECT * FROM m_accounts 
              WHERE emp_no LIKE ? AND full_name LIKE ? 
              ORDER BY date_updated DESC
              OFFSET ? ROWS 
              FETCH NEXT ? ROWS ONLY";

    $stmt = $conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
    $stmt->bindValue(1, $emp_no_search . '%', PDO::PARAM_STR);
    $stmt->bindValue(2, $full_name_search . '%', PDO::PARAM_STR);
    $stmt->bindValue(3, $page_first_result, PDO::PARAM_INT);
