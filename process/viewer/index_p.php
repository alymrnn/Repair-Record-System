...
if ($method == 'load_viewer_defect_table_data_last_page') {
    ...
    $number_of_result = intval(count_viewer_defect_table_data($conn));
...
if ($method == 'count_viewer_defect_table_data') {
    echo count_viewer_defect_table_data($conn);
}
...
//     $query = "SELECT * FROM t_defect_record_f WHERE (BINARY qc_status = 'Verified' OR BINARY record_type = 'White Tag')";
//     $stmt = $conn->prepare($query);
//     $stmt->execute();
