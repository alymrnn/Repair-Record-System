<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#search_car_maker').change(function () {
            ...
            if (selectedMaker) {
                $.ajax({
                    ...
                    success: function (response) {
                        $('#search_car_model').html(response).prop('disabled', false);
                        $('#qr_scan_pd').val('').prop('disabled', true);
                    }
                });
            } else {
                $('#search_car_model').html('<option></option>').prop('disabled', true);
                $('#qr_scan_pd').prop('disabled', false);
            }
        });

        $('#search_car_model').change(function () {
            ...
            if (e.which === 13) {
                ...
                if (window.qrSettings) {
                    ...
                    if (qrCode.length === totalLength) {
                        ...
                        document.getElementById('search_product_name').value = productName;
                        document.getElementById('search_lot_no').value = lotNo;
                        document.getElementById('search_serial_no').value = serialNo;
    ...
    const fetch_opt_car_maker_insp = () => {
        $.ajax({
            ...
            success: function (response) {
                $('#search_car_maker').html(response);
            }
        });
    }
    ...
    function handle_line_no_change(line_no) {
        ...
        if (record_type_update.includes(record_type)) {
            ...
            $("#qr_scan").prop('disabled', false).css('background-color', '');
        } else if (record_type === "Mancost Only") {
            update_car_maker(line_no);
            ...
            $("#car_model").prop('disabled', true).css('background-color', '#D3D3D3');
    ...
    const clear_search_input = () => {
        ...
        document.getElementById("search_car_model").value = '';
        document.getElementById("qr_scan_pd").value = '';

        document.getElementById("search_car_model").disabled = true;
    ...
    document.getElementById("line_no_pd_update").addEventListener("input", function () {
        ...
        update_car_maker_based_on_line_no(line_no);
    });
    ...
    // Function to fetch and update the car maker based on line_no
    function update_car_maker_based_on_line_no(line_no) {
    ...
    function handleCarMakerChange(selectOpt) {
        ...
        var car_model_input = document.getElementById('car_model');
        var qr_scan_input = document.getElementById('qr_scan');
        var product_name_input = document.getElementById('product_name');
        var lot_no_input = document.getElementById('lot_no');
        var serial_no_input = document.getElementById('serial_no');
    ...
    $(document).on('change', '#car_model', function () {
        ...
        var lot_no_input = $('#lot_no');
        var serial_no_input = $('#serial_no');
        var selectedCarModel = $(this).val();
        var carMaker = $('#car_maker').val();
